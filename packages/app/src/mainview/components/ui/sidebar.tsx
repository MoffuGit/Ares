        ...
        >
            ...
            <div
                ...
                className={cn(
                    ...
                    !isResizing && "transition-[width] duration-250 ease-out-cubic",
                ...
                >
                    <div
                        ...
                        className={cn(
                            ...
                            !isResizing && "transition-all duration-250 ease-out",
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
    const { toggleSidebar, setWidth, setOpen, setIsResizing } = useSidebar()
    ...
    const handlePointerDown = React.useCallback(
        (e: React.PointerEvent) => {
            ...
            const handlePointerMove = (ev: PointerEvent) => {
                ...
                const newWidth = Math.min(
                    SIDEBAR_WIDTH_MAX,
                    Math.max(SIDEBAR_WIDTH_MIN, startWidth.current + delta)
                )
                setWidth(newWidth)
                setOpen(true)
            }
        ...
        [setWidth, setOpen, setIsResizing]
    )
    ...
    return (
        <button
            ...
            className={cn(
                "absolute inset-y-0 z-20 hidden w-1  group-data-[side=left]:-right-0.5 group-data-[side=right]:left-0 after:absolute after:my-3 after:inset-y-0 after:start-1/2 after:w-[2px] opacity-0 hover:opacity-100 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
                ...
                "[[data-side=left][data-collapsible=offcanvas]_&]:right-0.5",
                "[[data-side=right][data-collapsible=offcanvas]_&]:-left-2",
                "after:bg-sidebar-border/90",
                className
            )}
            {...props}
        />
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
    return (
        <div
            ...
            className={cn(
                "flex mr-1 min-h-0 pt-1 flex-1 flex-col gap-0 overflow-auto group-data-[collapsible=icon]:overflow-hidden scroll-smooth",
                className
            )}
            {...props}
        />
...
function SidebarGroup({ className, ...props }: React.ComponentProps<"div">) {
    return (
        <div
            ...
            data-sidebar="group"
            className={cn("relative flex w-full min-w-0 flex-col pr-1", className)}
            {...props}
        />
    )
}
...
function SidebarGroupLabel({
    ...
    return useRender({
        ...
        props: mergeProps<"div">(
            {
                className: cn(
                    "flex my-2 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
                    className
                ),
            },
            props
...
const sidebarMenuButtonVariants = cva(
    "peer/menu-button active:scale-[0.98] group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent/60 active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent/40 dark:data-active:bg-sidebar-accent/60 data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
    {
        variants: {
            variant: {
