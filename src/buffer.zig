...
pub const PatchList = struct {
    ...
    pub fn push(self: *PatchList, range: Rngu64, replace: []const u8, arena: Allocator) !void {
        ...
        const copy = try arena.dupe(u8, replace);
        patch.* = .{
            .range = range,
            .replace = copy,
        };
...
const Info = struct {
    ...
    pub fn init(buffer: []u8, arena: Allocator) !Info {
        var lines: u64 = 0;

        for (buffer, 1..) |char, cnt| {
            if (char == '\n' or cnt == buffer.len) lines += 1;
        }

        var line_ranges = try arena.alloc(math.Rngu64, lines);

        var line_idx: usize = 0;
        var max_line_size: u64 = 0;
        var start: u64 = 0;
        for (buffer, 1..) |c, idx| {
            if (c == '\n' or idx == buffer.len) {
                const range: Rngu64 = .new(start, idx - 1);
                line_ranges[line_idx] = range;

                max_line_size = @max(max_line_size, range.dim());
                line_idx += 1;
                start = idx;
            }
        }
        ...
        return .{
            .lines_count = lines,
            .lines_max_size = max_line_size,
            .lines_ranges = line_ranges,
        };
    }
};
...
test "Basic Patch Operations" {
    ...
    const info = try Info.init(buffer, arena);

    try testing.expectEqualSlices(math.Rngu64, &expected, info.lines_ranges);
    try testing.expectEqual(info.lines_max_size, 70 - 33);
...
test "Patch Replace Different Size" {
    ...
    var patch_list: PatchList = .{};
    try patch_list.push(.new(19, 20), "LOTS OF ", arena);
...
test "Patch Overlapping Twice" {
    ...
    var patch_list: PatchList = .{};
    try patch_list.push(.new(0, 6), "patched", arena);
    try patch_list.push(.new(0, 7), "normal patched twice", arena);
