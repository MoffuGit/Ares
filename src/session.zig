...
pub fn init(self: *Session, _: Context(Session), io: Io, alloc: Allocator, conn: zqlite.Conn) !void {
    const old = bkl: {
...
test "Session init stores current id and reads previous id" {
    ...
    try first.init(undefined, io, allocator, conn);
    ...
    try second.init(undefined, io, allocator, conn);
