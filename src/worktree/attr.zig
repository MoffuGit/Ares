...
pub const BulkAttr = if (!supported) struct {} else struct {
    ...
    pub fn next(self: *BulkAttr) AttError!?Entry {
        outer: while (true) {
            ...
            var entry = Entry{ .name = &.{}, .kind = .unknown, .meta = .{} };
...
pub const Mask = packed struct(u32) {
    ...
    pub inline fn any(self: Mask) bool {
        return @as(u32, @bitCast(self)) != 0;
    }

    pub inline fn toInt(self: Mask) u32 {
        return @bitCast(self);
    }

    pub inline fn fromInt(v: u32) Mask {
        var m: Mask = @bitCast(v);
        m._reserved = 0;
        return m;
    }
};
